    ...
    def pad(self):
        ...
        if self.counter >= self.length:
            return copy_pool, copy_pool_init_state, copy.deepcopy(self.latent_goals), \
                   copy.deepcopy(self.latent_goals_init), copy.deepcopy(self.latent_obstacles), copy.deepcopy(
                self.latent_obstacles_sizes)
...
class MatchSampler:
    ...
    def update(self, initial_goals, desired_goals, initial_goals_latents=None, desired_goals_latents=None, planned_goals=None):
        ...
        for i in range(len(achieved_pool)):
            obs = [goal_concat(achieved_pool_init_state[i], achieved_pool[i][j]) for j in range(achieved_pool[i].shape[
                                                                                                    0])]  # obervation from first state of trajectoy to goal, which is the achieved state in the trajectory
            ...
            for j in range(len(desired_goals)):
                if self.args.graph:
                    ...
                    res = np.sqrt(np.sum(np.square(achieved_pool[i] - desired_goals[j]), axis=1)) - achieved_value[
                        i] / (self.args.hgg_L / self.max_dis / (1 - self.args.gamma))  # Todo: that was original
        ...
        match_count = self.match_lib.cost_flow(0,
                                               n)  # this library chooses for each j of our distributions the trajectory thath minimizes w
        ...
        # print("explore: " + str(explore_goals))
        # print("planned: " + str(planned_goals))
        self.pool = np.array(explore_goals)
        # print("pool: " + str(self.pool))
...
class HGGLearner:
    ...
    def learn(self, args, env, env_test, agent, buffer, write_goals=0, epoch=None, cycle=None):
        ...
        # store path of RRT*FND as goals to explore
        ...
        while len(planned_paths) < args.episodes:
            ...
            planned_paths = planned_paths + planned_path
            # print(args.episodes)
        ...
        if not self.stop:
            if args.vae_dist_help:
                if self.learn_calls > 0:
                    self.sampler.update(initial_goals, desired_goals, initial_goals_latents, desired_goals_latents, planned_path)
                else:
                    self.sampler.update(initial_goals, desired_goals, None, None, planned_goals=planned_path)
            else:
        ...
        for i in range(args.episodes):
            ...
            #     explore_goals = explore_goals + path_rrt
        ...
        for i in range(self.args.episodes):
            ...
            if goal_distance(achieved_trajectories[i][0],
                             achieved_trajectories[i][-1]) > 0.01:  # todo?? use here distance as well?
